...
# ----------------- MUSIC (pytgcalls 2.x.x New Syntax) -----------------
...
async def play_handler(client, message):
    ...
    try:
        ...
        # pytgcalls 2.x.x အတွက် အသစ်ပြောင်းလဲထားသော MediaStream ရေးထုံး
        await call_py.play(
            message.chat.id,
            MediaStream(
                url,
...
async def stop_handler(client, message):
    try:
        # pytgcalls 2.x.x တွင် leave_group_call အစား play_on_group သို့မဟုတ် drop_call သုံးရပါသည်
        await call_py.drop_call(message.chat.id)
